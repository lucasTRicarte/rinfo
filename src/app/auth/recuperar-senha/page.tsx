  ...
  return (
    <div className="min-h-screen bg-[#002C63] flex flex-col items-center justify-center px-4 py-12">
      ...
      <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl p-8">
        <Link href="/auth/login" className="flex items-center gap-1.5 text-sm text-gray-400 hover:text-[#003E8A] transition-colors mb-6">
          <ArrowLeft size={14} /> Voltar ao login
        </Link>

        <div className="mb-7 text-center">
          <h1 className="text-2xl font-black text-[#002C63]">Recuperar senha</h1>
          <p className="text-gray-500 text-sm mt-1">Enviaremos um link para redefinir sua senha</p>
        </div>

        <form action={action} className="space-y-4">
          {state?.error && (
            <div className="bg-red-50 text-red-700 text-sm px-4 py-3 rounded-xl border border-red-100">{state.error}</div>
          )}
          {state?.success && (
            <div className="bg-green-50 text-green-700 text-sm px-4 py-3 rounded-xl border border-green-100">{state.success}</div>
          )}

          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-1.5">E-mail cadastrado</label>
